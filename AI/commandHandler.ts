 ...
 * @param {Contact[]} contacts - User's contact list
 * @returns {Promise<CommandResult>} Processed command result
 */
export async function handleCommand(models: AIModels, prompt: string, contacts: Contact[] = []): Promise<CommandResult> {
...
`;
    ...
    try {
        ...
        return parseGeminiResponse(text) as CommandResult;
    } catch (error) {
